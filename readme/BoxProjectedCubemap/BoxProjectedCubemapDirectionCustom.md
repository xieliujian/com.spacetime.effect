...
> **关键：`t` 是整条射线的参数，不是「沿某轴走的距离」**
>
> `t_xMax = (boxMax.x - worldPos.x) / nrdir.x` 的推导来自射线方程的 x 分量：
>
> ```
> 命中点.x = worldPos.x + t × nrdir.x = boxMax.x
>             ↓ 解出 t
> t = (boxMax.x - worldPos.x) / nrdir.x
> ```
>
> 这个 `t` 是「沿 **整个** `nrdir` 方向走多少步，x 分量恰好碰到 boxMax.x」。  
> 把它代回射线方程，三个分量会**同时到位**：
>
> ```
> 命中点 = worldPos + t × nrdir
> 命中点.x = worldPos.x + t × nrdir.x = boxMax.x   ✓ (x 刚好撞墙)
> 命中点.y = worldPos.y + t × nrdir.y              (y 跟着 nrdir 自然移动)
> 命中点.z = worldPos.z + t × nrdir.z              (z 跟着 nrdir 自然移动)
> ```
>
> 所以每个轴算出的 `t` 都是**同一套射线方程里的同一个参数**，只是从不同分量约束推导出来的。
...
`fa` 是一个**标量**（没有方向），`nrdir` 是一个**三维方向向量**。  
`nrdir × fa` 是**向量的标量乘法**：给 `nrdir` 的每个分量都乘以 `fa`，结果是一个方向与 `nrdir` 相同、「步数」为 `fa` 的位移向量。

```
nrdir × fa = (nrdir.x × fa,  nrdir.y × fa,  nrdir.z × fa)
              ↑ 仍然有 x/y/z 三个分量，方向来自 nrdir，大小来自 fa
```

代入射线方程，三轴同时移动，得到交点的世界坐标：

```
交点 = worldPos + nrdir × fa
     = (worldPos.x + nrdir.x × fa,
        worldPos.y + nrdir.y × fa,
        worldPos.z + nrdir.z × fa)
```

继续用上面天花板的例子，设具体数字：

```
worldPos   = (2,  0, 3)     当前像素世界坐标
nrdir      = (0.6, 0.8, 0)  反射方向（斜向右上，已归一化）
boxMax     = (10, 1.2, 10)  盒子最大角，天花板在 y = 1.2
fa         = 1.5            t_y 最小，天花板最先被碰到
```

代入：

```
交点.x = 2   + 0.6 × 1.5 = 2.9       ← 在天花板面上，x 坐标在盒子范围内
交点.y = 0   + 0.8 × 1.5 = 1.2       ← 等于 boxMax.y，恰好在天花板平面上
交点.z = 3   + 0   × 1.5 = 3         ← 在天花板面上，z 坐标在盒子范围内
```

**交点 = (2.9, 1.2, 3)，这个点就落在天花板上。**

---

「撞到天花板」不是「三轴都碰到墙角」，而是「到达了天花板所在的那个平面（y = 1.2）」。  
天花板是一块面，它的 x 和 z 可以是盒子范围内的任意值。  
交点的 x = 2.9、z = 3 只是说明射线打在了天花板某个位置，**这完全正常**。

正是射线在 t = 1.5 时、沿 nrdir 方向整体移动后的真实三维位置。
